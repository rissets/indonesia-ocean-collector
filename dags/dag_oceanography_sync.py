...
def _enrich_day_window(target_date: str) -> int:
    """Fill sparse columns from nearby same-day rows across all sources."""
    ...
    WITH month_bucket AS (
        SELECT
            ...
            max(kecepatan_arus) FILTER (WHERE kecepatan_arus IS NOT NULL) AS kecepatan_arus
        FROM master_oceanography
        WHERE date_trunc('month', tanggal)::date = date_trunc('month', %(target)s::date)::date
        GROUP BY 1, 2, 3
    )
    UPDATE master_oceanography t
    SET
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute(day_sql, {"target": target_date})
                day_updated = max(cur.rowcount, 0)
                cur.execute(month_sql, {"target": target_date})
...
def _sync_yesterday(**context) -> dict:
    ...
    try:
        cmems_df = cmems_collector.collect_all(
            target_date, target_date, max_records=max(1000, BATCH_SIZE // 2), raw_dir=RAW_DIR
        )
    ...
    try:
        results["enriched_rows"] = _enrich_day_window(target_date)
    except Exception as exc:
        logger.error("Enrichment sync failed for %s: %s", target_date, exc)
